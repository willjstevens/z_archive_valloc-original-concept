...
import com.valloc.service.AbstractAgentService;
 ...
 * @author wstevens
 */
//public class UserServiceBase extends AbstractService implements UserService
public class UserServiceBase extends AbstractAgentService implements UserService
{
	@Override
	public String name() {
	...
	public Result asyncRemoteMeth(final String someArg) {
		final AsynchronousClientController controller = getControllerComponentFactory().newClientRemoteAsyncController(ServiceDirectory.USER, CommandDirectory.ASYNC_REMOTE_METH, this);
	...
	public Result asyncLocalMeth(final String someArg) {
		final AsynchronousClientController controller = getControllerComponentFactory().newClientLocalAsyncController(ServiceDirectory.USER_LOCAL, CommandDirectory.ASYNC_LOCAL_METH, this);
		controller.addArgLast(someArg);
		controller.submitAsync();
	...
	public Result interruptMe(final boolean isSync) {
		...
		if (isSync) {
			...
			final SynchronousClientController controller = getControllerComponentFactory().newClientRemoteSyncController(ServiceDirectory.USER, CommandDirectory.LONG_RUNNING_METH, this);
			...
			final AsynchronousClientController controller = getControllerComponentFactory().newClientRemoteAsyncController(ServiceDirectory.USER, CommandDirectory.LONG_RUNNING_METH, this);
			controller.addArgLast(isSync);
			controller.submitAsync();
			for (int i = 0; i < 3; i++) {
